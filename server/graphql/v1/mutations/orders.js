...
export async function createOrder(order, loaders, remoteUser, reqIp) {
  ...
  try {
    ...
    if (tier && order.collective.HostCollectiveId) {
      ...
      taxPercent = get(hostCollective, `settings.tiersTaxes.${tier.type}.percentage`, 0);
    ...
    if (tier && tier.amount && !tier.presets) {
      const expectedAmount = order.quantity * Math.trunc(tier.amount * (1 + taxPercent / 100));
    ...
    if (tier && tier.presets) {
      ...
      const minExpectedAmount = Math.trunc(minValue * (1 + taxPercent / 100));
